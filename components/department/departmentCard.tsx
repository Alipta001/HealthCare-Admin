...
type Department = {
  _id: string; // Changed to match your usage in useEffect/map
  id: number;
  name: string;
  description: string;
  totalDoctors: number;
  ...
  return (
    <>
      ...
      >
        ...
        <div className="absolute inset-0 opacity-0 group-hover:opacity-100 transition duration-500 pointer-events-none">
          <div className="absolute -top-20 -right-20 w-60 h-60 bg-white/30 rounded-full blur-3xl" />
        </div>

        <div className="relative z-10">
          {/* Text changed to slate-900 for better contrast on #cecfd1 */}
          ...
          <div className="mt-8 flex items-center justify-between">
            <div>
              ...
              {/* Darkened the gradient text slightly for readability */}
          ...
          <div className="mt-10 flex gap-4">
            ...
            <button
              ...
              className="flex-1 py-3 text-center rounded-2xl
                ...
                hover:scale-[1.03] hover:bg-slate-800
                active:scale-[0.97]
                transition-all duration-200 cursor-pointer"
            >
              + Add Doctor
