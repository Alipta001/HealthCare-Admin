      ...
      <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-md p-4 sm:p-6">
        <div className="relative w-full max-w-2xl bg-white rounded-3xl shadow-[0_40px_80px_rgba(0,0,0,0.35)] p-6 sm:p-10 overflow-y-auto max-h-[90vh]">
          ...
          <button
            type="button"
            onClick={onClose}
            className="absolute top-4 right-4 w-9 h-9 rounded-full bg-slate-100 hover:bg-red-500 hover:text-white flex items-center justify-center text-slate-500 transition cursor-pointer"
          >
            ✕
          </button>
          ...
          <DoctorModalForm doctor={doctor} departmentId={departmentId} departmentName={departmentName} onCancel={onClose} onClose={onClose}/>
        </div>
      </div>
  );
}
